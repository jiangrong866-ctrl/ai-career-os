...
$script = "D:\.codex专属工作间\ai-career-os\scripts\run_v3.py"
$workdir = "D:\.codex专属工作间\ai-career-os\scripts"
$resultPath = "D:\.codex专属工作间\ai-career-os\logs\v3_task_register_result.json"
...
try {
  ...
  Register-ScheduledTask -TaskName $taskName -Action $action -Trigger @($triggerDaily, $triggerLogon) -Settings $settings -Principal $principal -Description "AI Career OS v3 daily unattended production run" -Force | Out-Null

  $task = Get-ScheduledTask -TaskName $taskName
  $result.created = $true
  $result.run_level_highest = ($task.Principal.RunLevel -eq "Highest")
  $result.logon_type = [string]$task.Principal.LogonType
  $result.daily_trigger = ($task.Triggers | Where-Object { $_.CimClass.CimClassName -like "*DailyTrigger*" }).Count -gt 0
  $result.logon_trigger = ($task.Triggers | Where-Object { $_.CimClass.CimClassName -like "*LogonTrigger*" }).Count -gt 0
} catch {
